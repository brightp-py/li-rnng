...
def main(args):
  ...
  berkeleyrule
  <unk>
  <unkal> <unking> <unkest> <cunk-> <unkER> <cunking> <cunkal> <cunked>...
  girls	101
  danced	5
  in	26217
  ...
  """
  # SentencePieceVocabulary from data.py
  ...
  # normally, we just input the dataset once and use it to train.
  ...
  # figure out whether we're training from a previous checkpoint, and load it
  # if so.
  if args.train_from == '':
  ...
  while epoch <= args.num_epochs and not is_early_stop():
    ...
    def try_batch_step(token_ids, action_ids, max_stack_size, subword_end_mask,
      ...
      logger.warning('Retrying by halfing batch sizes...')
