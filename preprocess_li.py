...
Unlike preprocess.py, this program will not save any form of syntax trees. It
will generate a vocabulary, account for unknown tokens, and save the token ids
for each sentence.
"""
...
def is_next_open_bracket(line, start_idx):
    for char in line[(start_idx + 1):]:
        if char == '(':
            ...
            return False
    raise IndexError(
        'Bracket possibly not balanced, open bracket not followed by closed '
        'bracket'
    )
...
def get_tags_tokens_lowercase(line):
    ...
    for i in range(len(line)):
        if i == 0:
            assert line[i] == '('    
        if line[i] == '(' and not(is_next_open_bracket(line, i)):
    ...
    for terminal in output:
        ...
        # each terminal contains a POS tag and word
...
def transform_to_subword_tree(line, sp):
    ...
    spans = list(zip(begin_idxs, end_idxs))
    ...
    while i <= max_idx:
        ...
        if line_strip[i] == '(':
            if is_next_open_bracket(line_strip, i): # open non-terminal
                ...
                i += 1
                # get the next open bracket, which may be a terminal or
...
def make_vocab(textfile, seqlength, minseqlength, lowercase, replace_num,
               vocabsize, vocabminfreq, unkmethod, jobs,
    ...
    with Pool(jobs) as pool:
        for tags, sent, sent_lower in pool.map(get_tags_tokens_lowercase, trees):
            ...
            if (len(sent) > seqlength and apply_length_filter) \
                or len(sent) < minseqlength:
                continue
...
def get_data(args):
    def convert(textfile, lowercase, replace_num, seqlength, minseqlength,
                outfile, vocab, sp, apply_length_filter=True, jobs=-1):
        dropped = 0
        num_sents = 0
